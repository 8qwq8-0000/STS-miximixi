...
const X_DIST := 75
const Y_DIST := 62.5
const PLACEMENT_RANDOMNESS := 12.5
const FLOORS := 15
const MAP_WIDTH := 7
const PATHS := 6
...
## 精英房是一种独立的房间类型：从这一层开始才会出现，而且不会连着两个。
const ELITE_TIER := 3
const ELITE_MIN_ROOM_ROW := 6
...
func _setup_random_room_weights() -> void:
	...
	random_room_type_weights[Room.Type.ELITE] = random_room_type_weights[Room.Type.EVENT] + ELITE_ROOM_WEIGHT
...
func _set_room_randomly(room_to_set: Room) -> void:
	...
	while rejected:
		...
		var campfire_on_13 := is_campfire and room_to_set.row == 12
		# 精英房要到中段才出现，而且不会连着两个；精英档空着时干脆不生成。
		var elite_too_early := is_elite and room_to_set.row < ELITE_MIN_ROOM_ROW
		var consecutive_elite := is_elite and has_elite_parent
		var elite_unavailable := is_elite and not battle_stats_pool.has_tier(ELITE_TIER)

		rejected = campfire_below_4 or consecutive_campfire or consecutive_shop
		rejected = rejected or campfire_on_13 or elite_too_early or consecutive_elite or elite_unavailable
		
	room_to_set.type = type_candidate

	if type_candidate == Room.Type.ELITE:
		room_to_set.battle_stats = battle_stats_pool.get_random_battle_for_tier(ELITE_TIER)
		if room_to_set.battle_stats:
			return

		# 兜底：精英档临时抽不到就当作普通怪房。
		room_to_set.type = Room.Type.MONSTER

	if type_candidate == Room.Type.MONSTER:
		room_to_set.battle_stats = _roll_monster_battle(room_to_set.row)
	
	if type_candidate == Room.Type.EVENT:
		room_to_set.event_scene = event_room_pool.get_random()


## 普通怪房间抽哪一档：前 3 层用第 0 档，之后用第 1 档。
## 精英房不在这里 —— 它是单独的房间类型，见 _set_room_randomly。
func _roll_monster_battle(row: int) -> BattleStats:
	var tier := 0 if row <= 2 else 1

	var battle := battle_stats_pool.get_random_battle_for_tier(tier)
	if battle:
		return battle

	return battle_stats_pool.get_random_battle_for_tier(0)
