...
## Most rewards are claimed the moment they are pressed, so the button removes
## itself. The card offers are different: they open a picker that can be
## skipped, so those buttons set this to false and are removed later, once
## something has actually been chosen.
@export var free_on_pressed := true
