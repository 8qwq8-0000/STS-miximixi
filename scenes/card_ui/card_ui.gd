...
## The drop zones this card is willing to take, with each zone swapped for the
## creature it stands for (the player's targeting area reports the player).
func get_accepted_targets() -> Array[Node]:
	var accepted: Array[Node] = []
	for target in targets:
		var resolved := _resolve_target(target)
		if resolved and card.accepts_target(resolved):
			accepted.append(resolved)
	return accepted


func _resolve_target(target: Node) -> Node:
	var substitute: Variant = target.get("target_node")
	return substitute if substitute is Node else target
...
func _on_mouse_entered() -> void:
	# With an overlapping hand the neighbours cover part of this card, so pull it
	# above them while the cursor is on it.
	z_index = 1
